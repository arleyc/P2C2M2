...
##########################################################
  cat(paste("Calculating metrics for", n_sTrees, "trees of the posterior distribution\n\n"))
...
#####################################################################
  cat("Calculating metrics for the trees of the post. predictive distribution\n\n")
 ...
 ############################
      ...
      # phytools::writeNexus(simTrees[[1]], file=paste("simtreeloc",j,sep=""))
...
#####################################
  cat("Calculating differences btw. the posterior and the post. predictive distribution\n\n")
  
  if (errBool) {
      	  cat(paste("Calculating differences btw.", errReps, "pods and the post. predictive distribution to estimate error rates\n\n"))
          }
  ...
  for (s in descrStats) {
    ...
    if (srtBool) {
      outD[[s]]$dif = statshelpers.diffrnce(outD[[s]]$post_dist$sorted, 
		...
		if (errBool) {
      		outD[[s]]$err = statshelpers.diffrnce(outD[[s]]$post_pred_dist$sorted[sample(dim(outD[[s]]$post_pred_dist$sorted)[1],1),], 
                                            outD[[s]]$post_pred_dist$sorted)
    		}
      ...
      outD[[s]]$dif = statshelpers.diffrnce(outD[[s]]$post_dist$unsort, 
      	...
      	if (errBool) {
      	  for (r in 1:errReps) {  
      		pod<-sample(dim(outD[[s]]$post_pred_dist$unsort)[1],1)
      		outD[[s]]$post_pred_pod$unsort <- t(replicate(dim(outD[[s]]$post_pred_dist$unsort)[1],outD[[s]]$post_pred_dist$unsort[pod,]))
      		outD[[s]]$err[[r]] = statshelpers.diffrnce(outD[[s]]$post_pred_pod$unsort[-pod,], 
                                            outD[[s]]$post_pred_dist$unsort[-pod,])
    	  }
    	}
