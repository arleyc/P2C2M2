...
function (diff, alpha, tail) {
  ...
  acrossG_Sum = rowSums(diff, na.rm=TRUE)
  acrossG_Mean = rowMeans(diff, na.rm=TRUE)
  ...
  subCol1 = c(apply(acrossG, MARGIN=2, mean, na.rm=TRUE))
  subCol2 = c(apply(acrossG, MARGIN=2, sd, na.rm=TRUE))
  subCol3 = c(sigSgns)
