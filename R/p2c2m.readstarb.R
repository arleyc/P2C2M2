...
if (as.numeric(beastVers) == 2.7) {
	
		parsedata <- read.starbeast2_7(inFn, apWd)
		assoc <- parsedata$asoc$dta
		loci  <- parsedata$loci$dta
		gTrees <- parsedata$gtre$dta
		ploidy <- parsedata$pldy$dta
		pTrees <- parsedata$ptre$dta
		sTrees <- parsedata$stre$dta
	} else {
...
#####################################
  if (debugBool) {
    ...
    if (length(sTrees) > 39) {
      ...
      pTrees = utils::head(pTrees, reduced)
      sTrees = utils::head(sTrees, reduced)
      gTrees = lapply(gTrees, utils::head, reduced)
    }
  }
