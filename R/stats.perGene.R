...
function (diff, alpha, tail) {
  ...
  subCol1 = c(apply(diff, MARGIN=2, mean, na.rm=TRUE))
  # Calculation of stdv
  subCol2 = c(apply(diff, MARGIN=2, sd, na.rm=TRUE))
  # Assignment of signif. values
  subCol3 = c(sigSgns)
  
  # subCol4 counts the number of trees used to calculate the metric, paste after subCol3
  # subCol4 = colSums(!is.na(diff))
